...
import com.asbresearch.betfair.esa.auth.AppKeyAndSession;
...
import com.asbresearch.betfair.esa.protocol.*;
...
@Slf4j
public class Client {
    private static final AtomicInteger esaClientCounter = new AtomicInteger(1);
    private static final String CRLF = "\r\n";
    public static final String MARKET_CHANGE_ID = "mktChange.id";
    public static final String START_TIME = "mktChange.startTime";
    ...
    public void start() throws InvalidCredentialException, ConnectionException, StatusException {
        synchronized (startStopLock) {
            ...
            Thread thread = new Thread(this::run, String.format("ESAClient-%d", esaClientCounter.getAndIncrement()));
            thread.start();
        ...
        try {
            if (processor.getStatus() == ConnectionStatus.SUBSCRIBED) {
                ...
                if (processor.getLastRequestTime() + keepAliveHeartbeat < System.currentTimeMillis()) {
                    ...
                    log.info("Last Request Time is longer than {}: Sending Keep Alive Heartbeat", keepAliveHeartbeat);
                    heartbeat();
                } else if (processor.getLastResponseTime() + timeout < System.currentTimeMillis()) {
                    log.info("Last Response Time is longer than {}: Sending Keep Alive Heartbeat", timeout);
                    heartbeat();
                }
            }
        } catch (Exception e) {
    ...
    private void sendLineImpl(String line) throws ConnectionException {
        try {
            ...
            //Forcibly break the socket which will then trigger a re-connect if configured
            disconnect();
